 ...
 */
class ZohoCRMAddNoteRequestService implements
    RequestServiceInterface
{
    ...
    public function sendRequestDataEntity(
        ...
        $api_request = $gateway_data->getAPI()
            ->getRecordInstance(
                $request_data->getModuleName(),
                $request_data->getParentId()
            );

        // Create a note object
        $parameters = ZCRMNote::getInstance(
            $api_request,
            null
        );

        // Populate request data into the note instance
        $this->translateZohoCRMAddNoteRequest(
            $parameters,
            $request_data
        );
    ...
    public function newResponseDataEntity(
        ...
        return new ZohoCRMAddNoteResponseData(
            $api_response,
            $gateway_name,
            $action
        );
     ...
     */
    private function translateZohoCRMAddNoteRequest(
        ZCRMNote $zcrm_note,
        ZohoCRMAddNoteRequestData $request_data
    ): void {

        // Translate data into Zoho fields
        $zcrm_note->setTitle(
            $request_data->getTitle()
        ); // to set the note title
