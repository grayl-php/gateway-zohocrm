<?php
    ...
    */
   class ZohoCRMUpsertContactRequestService implements RequestServiceInterface
   {
                                              ...
                                              $request_data ): object
      {
         ...
         $parameters = ZCRMRecord::getInstance( "Contacts",
                                                null );

         // Populate request data into the contact instance
         $this->translateZohoCRMUpsertContactRequest( $parameters,
                                                      $request_data );

         // Send the request
         $bulk_response = $api_request->upsertRecords( [ $parameters ],
                                                       null,
                                                       null,
                                                       [ 'Email' ] );
                                              ...
                                              array $metadata ): object
      {
         ...
         return new ZohoCRMUpsertContactResponseData( $api_response,
                                                      $gateway_name,
                                                      $action );
      }
       ...
       */
      private function translateZohoCRMUpsertContactRequest ( ZCRMRecord $zcrm_contact,
                                                              ZohoCRMUpsertContactRequestData $request_data ): void
      {

         // Translate data into Zoho fields
         $zcrm_contact->setFieldValue( "Email",
                                       $request_data->getEmailAddress() );
         $zcrm_contact->setFieldValue( "Last_Name",
                                       $request_data->getName() );

         // Loop through each contact field and set it into the Zoho contact record
         foreach ( $request_data->getContactFields() as $key => $value ) {
            // Set the field
            $zcrm_contact->setFieldValue( str_replace( ' ',
                                                       '_',
                                                       $key ),
                                          $value );
         }
